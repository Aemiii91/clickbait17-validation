(function () {
	...
	function addToQueue(queue, file) {
	  queue.push(fetch(file.url).then(function (response) {
	...
	function parseLines(lines) {
	  return lines.map(function (line, i) {
	    ...
	    if (line) {
	      item = JSON.parse(line); // add truth item to data.lookup
	...
	function randBlankString(min, max) {
	  ...
	  return new Array(count).join("\xA0");
	}
	...
	function viewArticle(index) {
	  ...
	  item.targetCaptions.forEach(function (caption, i, captions) {
	    if (captions.indexOf(caption) == i) {
	      captionTags += "<span>".concat(caption, "</span>");
	    } else {
	      console.log('double!', caption);
	    }
	  });
	  content.innerHTML = "\n        ".concat(mediaTag, "\n        <section>\n            <h1 class=\"article-header\">").concat(item.targetTitle, "</h1>\n            <div class=\"article-body\"></div>\n            <footer>").concat(captionTags, "</footer>\n        </section>\n    ");
